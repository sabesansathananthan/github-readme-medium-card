...
app.get("/getMediumBlogs", async (request, response) => {
  try {
    ...
    result = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
              ...
              viewBox="0 0 ${ (((Math.round(limit/2))*height)+config.default.margin_top*2+config.card.spacing*(Math.floor(limit/2))) } ${((limit==1)?width:2*width)+config.default.margin_left+config.card.spacing}">`;
